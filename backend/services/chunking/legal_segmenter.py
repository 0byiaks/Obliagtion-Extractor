...
class LegalDocumentSegmenter:
    ...
    def __init__(self):
        ...
        self.legal_patterns = {
            ...
            # Section headings
            'section_heading': re.compile(r'^(\d+)\.\s+([A-Z][A-Z\s]+)$', re.MULTILINE),
    ...
    def segment_document(self, text: str, document_id: str = "document") -> List[LegalClause]:
        ...
        try:
            ...
            # First, detect sections using regex patterns
            ...
            for section in sections:
                section_clauses = self._process_section(
                    section, text, document_id, current_position
                )
            ...
            if not clauses:
                clauses = self._process_single_document(text, document_id)
